...
interface AppState extends Omit<BaseAppState, "user" | "club"> {
  ...
  register: (clubData: RegisterPayload) => Promise<void>;
...
function convertApiClub(apiClub: ApiClub): Club {
  return {
    ...apiClub,
    created_at: apiClub.created_at || new Date().toISOString(),
  };
}
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        // ✅ UPDATED: club + email + password
        login: async (club, email, password) => {
          set({ isLoading: true, error: null });
          try {
            console.log("🔐 Attempting login for:", { club, email });
            ...
            const clubObj = convertApiClub(response.club);
        ...
        // ✅ UPDATED: register includes consent fields
        ...
        addSupporter: (supporter) => set((state) => ({ supporters: [...state.supporters, supporter] })),
        updateSupporter: (id, updates) =>
          set((state) => ({
